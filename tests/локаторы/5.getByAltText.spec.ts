...
test.describe('Базовые тесты для getByAltText()', () => {
  ...
  test('Найти изображение по точному alt-тексту', async ({ page }) => {
    const landscapeImage = page.getByAltText('Красивый пейзаж с горами и озером', { exact: true });
    await expect(landscapeImage).toBeVisible();
    await expect(landscapeImage).toHaveAttribute('width', '300');
    await expect(landscapeImage).toHaveAttribute('height', '200');
  });
